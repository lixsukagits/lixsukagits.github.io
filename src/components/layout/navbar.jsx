  ...
  return (
    <span style={{ display: 'inline-flex', width: 18, height: 18, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <AnimatePresence mode="wait">
        {isDark ? (
          <motion.svg key="moon" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="none"
            initial={{ opacity: 0, rotate: -30, scale: 0.7 }} animate={{ opacity: 1, rotate: 0, scale: 1 }}
            exit={{ opacity: 0, rotate: 30, scale: 0.7 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
...
function SidebarSearch() {
  ...
  const handleSelect = (to) => { navigate(to); setQuery(''); setResults([]) }
  return (
    <div className="relative px-3 pb-2">
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
        ...
        <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari halaman..."
          className="flex-1 bg-transparent outline-none text-sm" style={{ color: 'var(--dark)', fontSize: '0.8rem' }} />
        {query && <button onClick={() => { setQuery(''); setResults([]) }} style={{ color: 'var(--body-color)' }}><X size={12} /></button>}
      </div>
      <AnimatePresence>
        {results.length > 0 && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
            ...
            {results.map(r => { const Icon = r.icon; return (
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(v => !v)}
        ...
        <span className="flex items-center gap-2"><Palette size={15} /><span>Tema Warna</span></span>
        <span className="w-4 h-4 rounded-full shrink-0" style={{ background: current.primary, border: '2px solid var(--border)' }} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            {COLOR_THEMES.map(theme => (
              <button key={theme.id} onClick={() => { setColorTheme(theme.id); setOpen(false) }}
                ...
                {theme.label}
                {theme.id === colorThemeId && <span className="ml-auto text-xs" style={{ color: 'var(--primary)' }}>✓</span>}
              </button>
            ))}
          </motion.div>
        )}
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm transition-colors hover:bg-[var(--primary-light)]"
        style={{ color: 'var(--body-color)' }}>
        <span className="flex items-center gap-2"><Globe size={15} /><span>{current.flag} {current.label}</span></span>
          ...
          <motion.div initial={{ opacity: 0, y: 6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            {LANGS.map(lang => (
              <button key={lang.code} onClick={() => { i18n.changeLanguage(lang.code); setOpen(false) }}
                ...
                {lang.code === i18n.language && <span className="ml-auto text-xs" style={{ color: 'var(--primary)' }}>✓</span>}
              </button>
            ))}
          </motion.div>
        )}
    ...
    <NavLink to={to} end={end}
      className={({ isActive }) =>
        `flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-colors relative group ${
          isActive ? 'text-[var(--primary)] bg-[var(--primary-light)]'
                   : 'text-[var(--body-color)] hover:text-[var(--dark)] hover:bg-[var(--primary-light)]'
        }`}>
      {({ isActive }) => (
        <>
          <motion.span animate={isActive ? { scale: 1.15, rotate: [0, -8, 8, 0] } : { scale: 1, rotate: 0 }}
            transition={{ type: 'spring', stiffness: 400, damping: 17 }} style={{ display: 'flex', flexShrink: 0 }}>
            <Icon size={16} />
          </motion.span>
          {label}
        </>
...
/* ─── LOGO — long press 2 detik = easter egg #7 ─────────────── */
function LogoLink({ to = '/', className, children }) {
  const pressTimer = useRef(null)
  const pressing   = useRef(false)
  const start = () => { pressing.current = true; pressTimer.current = setTimeout(() => { if (pressing.current) triggerEasterEgg('longpress') }, 2000) }
  const end   = () => { pressing.current = false; clearTimeout(pressTimer.current) }
  return (
    <NavLink to={to} className={className} style={{ WebkitUserSelect: 'none' }}
...
/* ─── THEME TOGGLE — dispatch event → disco counter (#5) ─────── */
function ThemeToggleBtn({ isDark, onToggle, ...props }) {
  const handle = () => { onToggle(); window.dispatchEvent(new CustomEvent('felix:theme-toggle')) }
  return <motion.button onClick={handle} whileTap={{ scale: 0.95 }} {...props} />
}
...
function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-60 shrink-0 h-screen sticky top-0 z-50"
      ...
      <div className="px-6 py-5 border-b shrink-0" style={{ borderColor: 'var(--border)' }}>
        <LogoLink to="/" className="font-display text-2xl font-extrabold" style={{ color: 'var(--primary)' }}>
          Felix<span style={{ color: 'var(--dark)' }}>.</span>
        </LogoLink>
        <p className="text-xs mt-0.5" style={{ color: 'var(--body-color)' }}>IT Enthusiast</p>
      </div>
      <div className="pt-3 shrink-0"><SidebarSearch /></div>
      ...
      <div className="px-4 py-4 border-t shrink-0 space-y-1.5" style={{ borderColor: 'var(--border)' }}>
        ...
        <ThemeToggleBtn isDark={isDark} onToggle={toggleTheme}
          ...
          <motion.span style={{
            ...
            width:28, height:16, borderRadius:8, flexShrink:0, transition:'background 0.3s', position:'relative',
            background: isDark ? 'var(--primary)' : 'var(--border)',
          }}>
            <motion.span layout animate={{ x: isDark ? 5 : -5 }} transition={{ type:'spring', stiffness:500, damping:30 }}
        ...
        <motion.a href="https://wa.me/6281262729243" target="_blank" rel="noopener noreferrer"
          ...
          whileHover={{ scale:1.02, boxShadow:'0 8px 24px rgba(55,88,249,0.35)' }} whileTap={{ scale:0.97 }}>
          Hubungi Saya
        </motion.a>
      </div>
    </aside>
  ...
  return (
    <>
      <header className="lg:hidden fixed top-0 left-0 w-full z-50 h-14 flex items-center justify-between px-5"
        ...
        <LogoLink to="/" className="font-display text-xl font-extrabold" style={{ color: 'var(--primary)' }}>
          Felix<span style={{ color: 'var(--dark)' }}>.</span>
        </LogoLink>
          ...
          <motion.div initial={{ x:'100%' }} animate={{ x:0 }} exit={{ x:'100%' }}
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b shrink-0" style={{ borderColor:'var(--border)' }}>
              ...
              <motion.button onClick={() => setOpen(false)} style={{ color:'var(--body-color)' }}
                whileTap={{ rotate:90, scale:0.85 }} transition={{ duration:0.2 }}><X size={20} /></motion.button>
            </div>
            <div className="pt-3 shrink-0"><SidebarSearch /></div>
            <nav className="flex-1 overflow-y-auto px-3 py-2 flex flex-col gap-0.5">
              {LINKS.map(({ to, icon, key }) => (
