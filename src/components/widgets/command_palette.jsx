...
export default function CommandPalette() {
  ...
  const actions = [
    { label: t('cmd.toggle_theme'), icon: theme === 'dark' ? '☀️' : '🌙', action: toggleTheme },
  ]
  ...
  return (
    <>
      ...
      >
        <Search size={13} /> <kbd className="opacity-60">Ctrl K</kbd>
      </button>
          ...
          <>
            ...
            >
              <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
                <Search size={16} style={{ color: 'var(--body-color)' }} />
                <input
                  ref={inputRef}
                  value={query}
                ...
                />
                <kbd className="text-xs px-2 py-1 rounded-md" style={{ background: 'var(--bg)', color: 'var(--body-color)' }}>ESC</kbd>
              </div>
