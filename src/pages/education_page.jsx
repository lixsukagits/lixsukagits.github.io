    ...
    <PageWrapper>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 md:px-10 py-20">
        ...
        <div className="relative mt-4">
          <div className="hidden sm:block absolute left-6 top-0 bottom-0 w-px"
            style={{ background: 'linear-gradient(to bottom, var(--primary), var(--border) 80%, transparent)' }} />

          <div className="space-y-6 sm:space-y-10">
            {education.map((ed, i) => {
              // FIX: achievements multilang
              const achievementsList = lang === 'en' ? (ed.achievementsEn ?? ed.achievements)
                                     : lang === 'zh' ? (ed.achievementsZh ?? ed.achievements)
                                     : ed.achievements
                ...
                >
                  ...
                  <div className="hidden sm:flex absolute left-0 top-5 flex-col items-center">
                    <motion.div
                      ...
                      style={{
                        border: ed.current ? '2px solid var(--primary)' : '2px solid var(--border)',
                    ...
                    {ed.current && (
                      <motion.div className="absolute w-12 h-12 rounded-2xl"
                  ...
                  {/* Card */}
                  <motion.div className="flex-1 card p-6 group relative overflow-hidden"
                    ...
                    <div className="relative">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                        <div className="flex items-center gap-3">
                          <div className="sm:hidden w-10 h-10 rounded-xl overflow-hidden shrink-0"
                            style={{ border: '1.5px solid var(--border)' }}>
                            <img src={ed.logo} alt={ed.school} className="w-full h-full object-cover" />
                          </div>
                          <div>
                            <h3 className="font-display font-bold text-lg sm:text-xl text-[var(--dark)]
                                           group-hover:text-[var(--primary)] transition-colors duration-200"
                              style={{ letterSpacing: '0.02em' }}>
                              {ed.school}
                            </h3>
                            <p className="font-semibold text-sm mt-0.5 text-[var(--primary)]">
                      ...
                      <div className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full mb-4 text-[var(--body-color)]"
                        style={{ background: 'var(--bg)' }}>
                        <MapPin size={11} /> {ed.location}
                      </div>

                      {/* FIX: pakai achievementsList multilang */}
                      <ul className="space-y-2">
                        {achievementsList.map((ac, j) => (
                          <motion.li key={j}
                            initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }}
                            viewport={{ once: true }}
                            transition={{ delay: i * 0.15 + j * 0.06 + 0.2 }}
                            className="flex items-start gap-2.5 text-sm text-[var(--body-color)]"
                            style={{ lineHeight: 1.65 }}>
                            <span className="shrink-0 mt-1 w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold"
                              style={{ background: 'rgba(55,88,249,0.1)', color: 'var(--primary)' }}>✦</span>
                            {ac}
                          </motion.li>
                        ))}
                      </ul>
